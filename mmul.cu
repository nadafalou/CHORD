...
int main( int argc, char *argv[] ) {
	...
	float *h_Iin = (float *)malloc(T * rho * sizeof(float));
	float *h_W = (float *)malloc(rho * B * sizeof(float));
	float *h_Iout = (float *)malloc(T * B * sizeof(float));
	...
	for (int i = 0; i < T * rho; i++) {
		h_Iin[i] = 0;
		if (i == 1 or i == 2 or i == 6) { h_Iin[i] = 1; }
	}
	for (int i = 0; i < rho * B; i++) {
		h_W[i] = 0;
		if (i == 2 or i == 5 or i == 7 or i == 14 or i == 15 or i == 18) { h_W[i] = 1; } 
	}

	// Copy data from CPU to GPU
	cudaMemcpy(d_Iin,h_Iin,T * rho * sizeof(float),cudaMemcpyHostToDevice);
	cudaMemcpy(d_W,h_W,rho * B * sizeof(float),cudaMemcpyHostToDevice);
	} else {
	// temp until real data is passed in
	// Fill the arrays A and B on GPU with random numbers
	GPU_fill_rand(d_Iin, T, rho);
	GPU_fill_rand(d_W, rho, B);
	...
	print_matrix(h_Iin, T, rho);
	std::cout << "W =" << std::endl;
	print_matrix(h_W, B, rho);
	...
	cudaMemcpy(h_Iout,d_Iout,T * B * sizeof(float),cudaMemcpyDeviceToHost);
	std::cout << "Iout =" << std::endl;
	print_matrix(h_Iout, T, B);
