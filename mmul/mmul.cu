...
int main( int argc, char *argv[] ) {
	...
	cublasStatus_t err;
	cudaError_t err2;
	if ((err2 = cudaDeviceSynchronize()) != cudaSuccess){
		std::cout << "cudaDeviceSynchronize failed: " << cudaGetErrorName(err2) << std::endl;
		return -1;
	}


	// NON-BATCHED
	int calls_per_stream = F / num_streams; 
	...
	std::cout << num_runs << " cublasHgemm operations on average finished in " << (time1 - time0) / (float) num_runs << "s" << std::endl;
	...
	std::cout << num_runs << " cublasHgemmBatched operations on average finished in " << (time3 - time2) / (float) num_runs << "s" << std::endl;
